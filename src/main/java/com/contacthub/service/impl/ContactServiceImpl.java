...
public class ContactServiceImpl implements ContactService {
	...
	public ContactResponse fetch(String email) {
		//
		return contactRepository.findByEmail(email).map(contactMapper::toResponse)
				.orElseThrow(() -> new IllegalArgumentException("A contact with this email doesn't exist: " + email));
	}

	@Override
	public ContactResponse update(ContactRequest request) {
		//
		var existing = contactRepository.findByEmail(request.email()).orElseThrow(
				() -> new IllegalArgumentException("A contact with this email doesn't exist: " + request.email()));
